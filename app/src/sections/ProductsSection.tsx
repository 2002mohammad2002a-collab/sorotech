...
// 1. تعريف الأنواع الثابتة خارج المكون
type Category = 'battery' | 'inverter' | 'solar';
...
// 2. نقل مصفوفة البيانات خارج المكون لتحسين الأداء ومنع إعادة الحجز في الذاكرة
const PRODUCTS_DATA: Product[] = [
  // أولاً: سلسلة بطاريات Tanker
  {
    id: 'tanker-series',
    category: 'battery',
  ...
  // ثانياً: سلسلة One
  {
    ...
    specs: [
      'تصميم ذكي ومدمج للغاية يدعم التعليق الجداري أو التثبيت الأرضي',
  ...
  // تاسعاً: إضافة عنصر ناصع للألواح الكهروضوئية (حتى لا يبقى التبويب مكسوراً وفارغاً)
  {
    id: 'soropanel-series',
    category: 'solar',
    title: 'سلسلة ألواح Sorotech N-Type الفائقة',
    models: 'القدرات المتوقعة: 580W | 620W Monocrystalline',
    specs: [
      'تقنية خلايا N-Type TOPCon المتطورة بأعلى كفاءة تحويل طاقة في السوق العالمية',
      'مقاومة فائقة لظاهرة التدهور المستحث بالضوء والحرارة العالية (PID Resistance)',
      'زجاج أمامي وخلفي مزدوج (Bifacial) لزيادة إنتاجية الطاقة من الانعكاس الأرضي'
    ],
    image: 'solar-placeholder.png',
    isComingSoon: true,
    techTable: {
      columns: ['المواصفة الكهربائية الأولية', 'SoroPanel 580W', 'SoroPanel 620W'],
      rows: [
        ['نوع الخلايا الداخلي', 'Monocrystalline N-Type TOPCon', 'Monocrystalline N-Type TOPCon'],
        ['كفاءة اللوح الشمسي الاستثنائية', 'Up to 22.6%', 'Up to 23.1%'],
        ['جهد الدائرة المفتوحة (Voc)', '51.4 V', '53.2 V'],
        ['تيار القصر الأقصى (Isc)', '14.2 A', '14.6 A'],
        ['معامل التدهور السنوي المضمون', '0.4% سنوياً على مدار 30 سنة ضمان أداء خطي', '0.4% سنوياً']
      ]
    }
  }
];
...
export default function ProductsSection() {
  ...
  // 3. منع التمرير (Body Scroll Lock) عند فتح الـ Modal للحفاظ على تجربة مستخدم نظيفة
  useEffect(() => {
    if (selectedProduct) {
  ...
  return (
    <section id="products" className="py-24 bg-zinc-50 overflow-hidden text-right" dir="rtl">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        {/* أزرار التصفية المعدلة الخالية من الـ Any */}
        <div className="flex flex-wrap justify-center gap-3 mb-16">
          {[
            { id: 'battery' as Category, label: 'بطاريات الليثيوم', icon: Cpu },
            { id: 'inverter' as Category, label: 'الإنفرترات الهجينة', icon: Zap },
            { id: 'solar' as Category, label: 'الألواح الشمسية', icon: Sun }
          ].map((tab) => {
            const Icon = tab.icon;
                ...
                <div>
                  <div className={`w-full h-48 flex items-center justify-center mb-6 overflow-hidden rounded-xl bg-zinc-50 border border-zinc-100 ${product.isComingSoon ? 'opacity-60' : ''}`}>
                    <img 
                      ...
                      onError={(e) => { 
                        // عرض أيقونة بديلة أو معالجة الفشل بشكل لا يكسر الـ Layout
        ...
        {/* نافذة عرض جدول المواصفات الفنية (Modal) */}
        <AnimatePresence>
          {selectedProduct && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
              ...
              >
                <button 
                  onClick={() => setSelectedProduct(null)}
                  className="absolute top-6 left-6 p-2 rounded-xl bg-zinc-100 text-zinc-600 hover:bg-red-600 hover:text-white transition-colors animate-none"
                >
                  <X className="w-5 h-5" />
                </button>
